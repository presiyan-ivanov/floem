...
pub fn tv_shows() -> impl View {
    let top_rated = include_str!("../../assets/data/tv_shows/top_rated.json");
    let popular = include_str!("../../assets/data/tv_shows/popular.json");
    let now_playing = include_str!("../../assets/data/tv_shows/currently_airing.json");

    let popular_movies: Page<TvShow> =
        serde_json::from_str(popular).expect("JSON was not well-formatted");

    let top_rated: Page<TvShow> =
        serde_json::from_str(top_rated).expect("JSON was not well-formatted");

    let now_playing: Page<TvShow> =
    ...
    scroll(
        v_stack((
            ...
            v_stack((
                label(move || "Top Rated Shows").class(CarouselTitle),
